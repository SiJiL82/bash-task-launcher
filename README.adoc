...
== Troubleshooting
=== Debug Logging
If you want to capture all the output from a task to assist with troubleshooting, run the `enable-debug` task. +
This task prompts for a file location to write the output to, in addition to logging to the terminal. +
This can be especially useful for capturing output from tasks such as Docker build jobs which can generate a lot of logging and then compress the output in the terminal, making it difficult to review for issues.

To disable logging, run the `disable-debug` task. +


*_Note:_* _These tasks do not appear on any menus, or in the shell autocomplete commands. They must be executed manually by the user._
